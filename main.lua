...
local boostThreshold = Config.BoostThreshold or 5
local useAPI = Config.UseAPI or false

-- BOOST MODE VARIABLES
local originalCPM = currentCPM
local originalErrorRate = errorRate
local originalThinkDelay = thinkDelayCurrent
local originalHumanize = useHumanization
local originalBlatant = isBlatant
...
TogglesFrame.Size = UDim2.new(1, 0, 0, 420)
...
local function UpdateLayout()
    if settingsCollapsed then
        ...
        Tween(SettingsFrame, {Size = UDim2.new(1, 0, 0, 575), Position = UDim2.new(0, 0, 1, -575)})
        Tween(ScrollList, {Size = UDim2.new(1, -10, 1, -695)})
...
local function GenerateKeyboard()
    ...
    space:FindFirstChildOfClass("TextLabel").Text = "SPACE"
end
...
LayoutDropdown.Position = UDim2.new(0, 150, 0, 175)
...
local CPMInput = Instance.new("TextBox", SlidersFrame)
CPMInput.Size = UDim2.new(0.4, -15, 0, 20)
CPMInput.Position = UDim2.new(0.6, 0, 0, 8)
CPMInput.BackgroundColor3 = THEME.Slider
CPMInput.Text = tostring(currentCPM)
CPMInput.TextColor3 = THEME.Text
CPMInput.Font = Enum.Font.GothamMedium
CPMInput.TextSize = 11
CPMInput.PlaceholderText = "CPM"
Instance.new("UICorner", CPMInput).CornerRadius = UDim.new(0, 4)
...
local pct = (currentCPM - MIN_CPM) / (max - MIN_CPM)
...
CPMInput.FocusLost:Connect(function()
    local value = tonumber(CPMInput.Text)
    if value then
        local max = isBlatant and MAX_CPM_BLATANT or MAX_CPM_LEGIT
        currentCPM = math.clamp(value, MIN_CPM, max)
        CPMInput.Text = tostring(currentCPM)
        SliderLabel.Text = "Speed: " .. currentCPM .. " CPM"
...
ErrorLabel.Size = UDim2.new(0.6, -15, 0, 18)
ErrorLabel.Position = UDim2.new(0, 15, 0, 40)
ErrorLabel.BackgroundTransparency = 1
ErrorLabel.TextXAlignment = Enum.TextXAlignment.Left

local ErrorInput = Instance.new("TextBox", SlidersFrame)
ErrorInput.Size = UDim2.new(0.4, -15, 0, 18)
ErrorInput.Position = UDim2.new(0.6, 0, 0, 40)
ErrorInput.BackgroundColor3 = THEME.Slider
ErrorInput.Text = tostring(errorRate)
ErrorInput.TextColor3 = THEME.Text
ErrorInput.Font = Enum.Font.GothamMedium
ErrorInput.TextSize = 10
ErrorInput.PlaceholderText = "Error %"
Instance.new("UICorner", ErrorInput).CornerRadius = UDim.new(0, 4)

local ErrorBg = Instance.new("Frame", SlidersFrame)
ErrorBg.Size = UDim2.new(1, -30, 0, 6)
ErrorBg.Position = UDim2.new(0, 15, 0, 60)
...
ErrorInput.FocusLost:Connect(function()
    local value = tonumber(ErrorInput.Text)
    if value then
        errorRate = math.clamp(value, 0, 30)
        ErrorInput.Text = tostring(errorRate)
        ErrorLabel.Text = "Error Rate: " .. errorRate .. "%"
        
        local pct = errorRate / 30
        ErrorFill.Size = UDim2.new(pct, 0, 1, 0)
...
ThinkLabel.Size = UDim2.new(0.6, -15, 0, 18)
ThinkLabel.Position = UDim2.new(0, 15, 0, 70)
ThinkLabel.BackgroundTransparency = 1
ThinkLabel.TextXAlignment = Enum.TextXAlignment.Left

local ThinkInput = Instance.new("TextBox", SlidersFrame)
ThinkInput.Size = UDim2.new(0.4, -15, 0, 18)
ThinkInput.Position = UDim2.new(0.6, 0, 0, 70)
ThinkInput.BackgroundColor3 = THEME.Slider
ThinkInput.Text = string.format("%.2f", thinkDelayCurrent)
ThinkInput.TextColor3 = THEME.Text
ThinkInput.Font = Enum.Font.GothamMedium
ThinkInput.TextSize = 10
ThinkInput.PlaceholderText = "Seconds"
Instance.new("UICorner", ThinkInput).CornerRadius = UDim.new(0, 4)

local ThinkBg = Instance.new("Frame", SlidersFrame)
ThinkBg.Size = UDim2.new(1, -30, 0, 6)
ThinkBg.Position = UDim2.new(0, 15, 0, 90)
...
ThinkInput.FocusLost:Connect(function()
    local value = tonumber(ThinkInput.Text)
    if value then
        thinkDelayCurrent = math.clamp(value, thinkDelayMin, thinkDelayMax)
        ThinkInput.Text = string.format("%.2f", thinkDelayCurrent)
        ThinkLabel.Text = string.format("Think: %.2fs", thinkDelayCurrent)
        
        local pct = (thinkDelayCurrent - thinkDelayMin) / (thinkDelayMax - thinkDelayMin)
        ThinkFill.Size = UDim2.new(pct, 0, 1, 0)
...
-- Boost Threshold Slider + Input
local BoostLabel = Instance.new("TextLabel", SlidersFrame)
BoostLabel.Text = "Boost at: " .. boostThreshold .. "s"
BoostLabel.Font = Enum.Font.GothamMedium
BoostLabel.TextSize = 11
BoostLabel.TextColor3 = THEME.SubText
BoostLabel.Size = UDim2.new(0.6, -15, 0, 18)
BoostLabel.Position = UDim2.new(0, 15, 0, 100)
BoostLabel.BackgroundTransparency = 1
BoostLabel.TextXAlignment = Enum.TextXAlignment.Left

local BoostInput = Instance.new("TextBox", SlidersFrame)
BoostInput.Size = UDim2.new(0.4, -15, 0, 18)
BoostInput.Position = UDim2.new(0.6, 0, 0, 100)
BoostInput.BackgroundColor3 = THEME.Slider
BoostInput.Text = tostring(boostThreshold)
BoostInput.TextColor3 = THEME.Text
BoostInput.Font = Enum.Font.GothamMedium
BoostInput.TextSize = 10
BoostInput.PlaceholderText = "Seconds"
Instance.new("UICorner", BoostInput).CornerRadius = UDim.new(0, 4)

local BoostBg = Instance.new("Frame", SlidersFrame)
BoostBg.Size = UDim2.new(1, -30, 0, 6)
BoostBg.Position = UDim2.new(0, 15, 0, 120)
BoostBg.BackgroundColor3 = THEME.Slider
Instance.new("UICorner", BoostBg).CornerRadius = UDim.new(1, 0)

local BoostFill = Instance.new("Frame", BoostBg)
BoostFill.Size = UDim2.new(boostThreshold/15, 0, 1, 0)
BoostFill.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
Instance.new("UICorner", BoostFill).CornerRadius = UDim.new(1, 0)

local BoostBtn = Instance.new("TextButton", BoostBg)
BoostBtn.Size = UDim2.new(1,0,1,0)
BoostBtn.BackgroundTransparency = 1
BoostBtn.Text = ""

SetupSlider(BoostBtn, BoostBg, BoostFill, function(pct)
    boostThreshold = math.floor(pct * 15)
    if boostThreshold < 1 then boostThreshold = 1 end
    Config.BoostThreshold = boostThreshold
...
BoostInput.FocusLost:Connect(function()
    local value = tonumber(BoostInput.Text)
    if value then
        boostThreshold = math.clamp(value, 1, 15)
        BoostInput.Text = tostring(boostThreshold)
        BoostLabel.Text = "Boost at: " .. boostThreshold .. "s"
        
        local pct = boostThreshold / 15
        BoostFill.Size = UDim2.new(pct, 0, 1, 0)
...
-- PANIC MODE TOGGLE (BARU)
local PanicModeBtn = CreateToggle("Panic Mode: "..(panicMode and "ON" or "OFF"), UDim2.new(0, 15, 0, 143), function()
    panicMode = not panicMode
    Config.PanicMode = panicMode
    
    -- Reset boost mode jika panic mode dimatikan
    if not panicMode and isBoostMode then
        isBoostMode = false
        currentCPM = originalCPM
        errorRate = originalErrorRate
        thinkDelayCurrent = originalThinkDelay
        useHumanization = originalHumanize
        isBlatant = originalBlatant
        ...
        StatusText.Text = "Panic Mode Disabled"
        StatusText.TextColor3 = THEME.Warning
        
        ShowToast("Panic/Boost Mode Disabled", "warning")
    end
    
    return panicMode, "Panic Mode: "..(panicMode and "ON" or "OFF"), panicMode and THEME.Success or Color3.fromRGB(255, 100, 100)
end)
PanicModeBtn.TextColor3 = panicMode and THEME.Success or Color3.fromRGB(255, 100, 100)
PanicModeBtn.Size = UDim2.new(0, 130, 0, 24)

local APIBtn = CreateToggle("Use API: "..(useAPI and "ON" or "OFF"), UDim2.new(0, 150, 0, 143), function()
    useAPI = not useAPI
    Config.UseAPI = useAPI
    return useAPI, "Use API: "..(useAPI and "ON" or "OFF"), useAPI and THEME.Success or Color3.fromRGB(255, 100, 100)
end)
APIBtn.TextColor3 = useAPI and THEME.Success or Color3.fromRGB(255, 100, 100)
APIBtn.Size = UDim2.new(0, 130, 0, 24)
...
function SmartType(targetWord, currentDetected, isCorrection, bypassTurn)
...
runConn = RunService.RenderStepped:Connect(function()
    local success, err = pcall(function()
        ...
        -- AUTO BOOST MODE (PANIC MODE)
        if panicMode and seconds then
            if seconds <= boostThreshold and not isBoostMode and isMyTurn and not isTyping then
                isBoostMode = true
                
                originalCPM = currentCPM
                originalErrorRate = errorRate
                originalThinkDelay = thinkDelayCurrent
                originalHumanize = useHumanization
                originalBlatant = isBlatant
                
                currentCPM = 1000
                errorRate = 0
                thinkDelayCurrent = 0.40
                useHumanization = false
                isBlatant = true
                
                SliderLabel.Text = "Speed: 1000 CPM ⚡ BOOST"
                SliderLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
                CPMInput.Text = "1000"
                ErrorLabel.Text = "Error Rate: 0% ⚡ BOOST"
                ErrorLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
                ErrorInput.Text = "0"
                ThinkLabel.Text = "Think: 0.40s ⚡ BOOST"
                ThinkLabel.TextColor3 = Color3.fromRGB(255, 50, 50)
                ThinkInput.Text = "0.40"
                
                StatusText.Text = "⚡ BOOST MODE ACTIVE ⚡"
                StatusText.TextColor3 = Color3.fromRGB(255, 50, 50)
                Tween(StatusDot, {BackgroundColor3 = Color3.fromRGB(255, 50, 50)})
                
                Tween(SliderFill, {BackgroundColor3 = Color3.fromRGB(255, 50, 50)}, 0.2)
                
                ShowToast("⚡ BOOST MODE: 1000 CPM!", "error")
                
            elseif seconds > boostThreshold and isBoostMode then
                isBoostMode = false
                ...
                StatusText.Text = "Normal Mode Restored"
                StatusText.TextColor3 = THEME.Success
                Tween(StatusDot, {BackgroundColor3 = THEME.Success})
                
                if currentCPM > 900 then 
                    Tween(SliderFill, {BackgroundColor3 = Color3.fromRGB(255,80,80)}, 0.2)
                else 
                    Tween(SliderFill, {BackgroundColor3 = THEME.Accent}, 0.2)
                end
        ...
        if isVisible and isMyTurn and not isTyping and seconds and seconds < 1.5 then
            ...
            if bucket then
                ...
                local detected = GetCurrentGameWord(frame)
